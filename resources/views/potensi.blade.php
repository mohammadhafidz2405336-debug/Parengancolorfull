                ...
                @if($potensiKiri)
                    <div class="bg-white rounded-3xl shadow-xs border border-slate-100 p-6 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center" data-aos="fade-right">
                        <div class="lg:col-span-5 h-72 sm:h-96 rounded-2xl overflow-hidden shadow-md relative">
                            <img src="{{ $potensiKiri->foto ? (Str::startsWith($potensiKiri->foto, ['http://', 'https://']) ? $potensiKiri->foto : asset('storage/' . $potensiKiri->foto)) : asset('images/placeholder.jpg') }}" 
                        ...
                        <div class="lg:col-span-7 space-y-4">
                            ...
                            <p class="text-slate-600 text-sm leading-relaxed text-justify">
                                {{ $potensiKiri->deskripsi ?? 'Belum ada deskripsi untuk potensi ini.' }}
                            </p>
                            @if($potensiKiri->kontak)
                            @php
                                $kontakKiri = preg_replace('/[^0-9]/', '', $potensiKiri->kontak);
                                if (str_starts_with($kontakKiri, '0')) { $kontakKiri = '62' . substr($kontakKiri, 1); }
                            @endphp
                            <div class="pt-2">
                                <a href="https://wa.me/{{ $kontakKiri }}" target="_blank" class="text-xs font-semibold text-emerald-700 bg-emerald-50 px-4 py-2 rounded-lg inline-flex items-center gap-2 transition-all hover:bg-emerald-100">
                ...
                @if($potensiKanan)
                    <div class="bg-white rounded-3xl shadow-xs border border-slate-100 p-6 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center" data-aos="fade-left">
                        <div class="lg:col-span-7 space-y-4 order-2 lg:order-1">
                            ...
                            <p class="text-slate-600 text-sm leading-relaxed text-justify">
                                {{ $potensiKanan->deskripsi ?? 'Belum ada deskripsi untuk potensi ini.' }}
                            </p>
                            @if($potensiKanan->kontak)
                            @php
                                $kontakKanan = preg_replace('/[^0-9]/', '', $potensiKanan->kontak);
                                if (str_starts_with($kontakKanan, '0')) { $kontakKanan = '62' . substr($kontakKanan, 1); }
                            @endphp
                            <div class="pt-2">
                                <a href="https://wa.me/{{ $kontakKanan }}" target="_blank" class="text-xs font-semibold text-emerald-700 bg-emerald-50 px-4 py-2 rounded-lg inline-flex items-center gap-2 transition-all hover:bg-emerald-100">
                                    <i class="fa-brands fa-whatsapp text-lg"></i> Hubungi Pemilik
                                </a>
                            </div>
                            @endif
                        ...
                        <div class="lg:col-span-5 h-72 sm:h-96 rounded-2xl overflow-hidden shadow-md relative order-1 lg:order-2">
                            <img src="{{ $potensiKanan->foto ? (Str::startsWith($potensiKanan->foto, ['http://', 'https://']) ? $potensiKanan->foto : asset('storage/' . $potensiKanan->foto)) : asset('images/placeholder.jpg') }}" 
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-xs overflow-hidden flex flex-col justify-between transform hover:-translate-y-2 hover:shadow-md transition-all duration-300">
                <div>
                    <div class="h-48 bg-slate-200 relative">
                        <img src="{{ $item->foto ? (Str::startsWith($item->foto, ['http://', 'https://']) ? $item->foto : asset('storage/' . $item->foto)) : asset('images/placeholder.jpg') }}" 
                             alt="{{ $item->nama }}" 
                    ...
                    <div class="p-6 space-y-2">
                        ...
                        <p class="text-xs text-slate-600 leading-relaxed pt-2 line-clamp-3">
                            {{ $item->deskripsi ?? 'Belum ada deskripsi untuk UMKM ini.' }}
                        </p>
                    </div>
                </div>
                ...
                <div class="p-6 pt-0 border-t border-slate-50 mt-4 flex flex-col gap-3">
                    ...
                    <div class="flex gap-2">
                        ...
                        @if($item->kontak)
                        @php
                            $kontakUmkm = preg_replace('/[^0-9]/', '', $item->kontak);
                            if (str_starts_with($kontakUmkm, '0')) { $kontakUmkm = '62' . substr($kontakUmkm, 1); }
                        @endphp
                        <a href="https://wa.me/{{ $kontakUmkm }}" target="_blank" class="flex-1 inline-flex items-center justify-center gap-1 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-3 py-2 rounded-xl transition shadow-2xs">
